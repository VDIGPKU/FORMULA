...
if __name__ == '__main__':
    ...
    parser.add_argument("--voc07_dir", type=str, default='datasets',
                        help="Path where the VOC2007 data are.")
    parser.add_argument("--voc12_dir", type=str, default='datasets',
    ...
    for year, image_set in sets:
        image_ids = open(f'{year2dir[year]}/VOCdevkit/VOC{year}/ImageSets/Main/{image_set}.txt').read().strip().split()
        print(f"==> Year: {year}, ImageSet: {image_set}, Number of images: {len(image_ids)}")
        data = []
        for image_id in image_ids:
            full_img_path = pathlib.Path(year2dir[year]) / "VOCdevkit" / f"VOC{year}" / "JPEGImages" / f"{image_id}.jpg"
            full_ann_path = pathlib.Path(year2dir[year]) / "VOCdevkit" / f"VOC{year}" / "Annotations" / f"{image_id}.xml"
