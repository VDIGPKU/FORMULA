# Copyright 2021 - VDIGPKU Comfort and Driving Assistance
#
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Prepares the FORMULA pseudo-boxes from a VOC"
                    "dataset in the data format expected from detectron2.")
    parser.add_argument("--voc_dir", type=str, default='datasets',
                        help="Path to where the VOC dataset is.")
    parser.add_argument("--year", type=str, default='2007', help="Year of VOC dataset.")
    parser.add_argument("--pboxes", type=str, default='../outputs/VOC07_trainval/FORMULA-vit_small16_k/preds.pkl',
                        help="Path to where the FORMULA CA pseudo boxes for the VOCyear trainval data are.")
    args = parser.parse_args()

    # Dataset directory
    # voc_dir = f"{args.voc_dir}{args.year}"
    voc_dir = f"{args.voc_dir}"

    # Load the boxes
    with open(args.pboxes, 'rb') as handle:
        FORMULA_pseudo_boxes = pickle.load(handle)

    data = []
    cnt = 0
    for image_name in FORMULA_pseudo_boxes:
        image_id = image_name[:-len('.jpg')]
        image_id_int = int(image_id)
        full_img_path = pathlib.Path(voc_dir) / "VOCdevkit" / f"VOC{args.year}" / "JPEGImages" / image_name
        full_ann_path = pathlib.Path(voc_dir) / "VOCdevkit" / f"VOC{args.year}" / "Annotations" / f"{image_id}.xml"
        ...
        data.append({
            ...
            "annotations": prepare_annotation_data(FORMULA_pseudo_boxes[image_name]),
        })
        cnt += 1
    print(f'Number images saved {cnt}')
    dataset_name = f"voc_{args.year}_trainval_FORMULA_CAD"
    json_data = {
        "dataset": data,
        "meta_data": {
            "dirname": voc_dir,
