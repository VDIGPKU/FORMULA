...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Prepares the FORMULA pseudo-boxes from a COCO2014"
    ...
    parser.add_argument("--pboxes", type=str, default='../outputs/COCO20k_train/FORMULA-vit_small16_k/preds.pkl',
                        help="Path to where the FORMULA CA pseudo boxes for the VOCyear trainval data are.")
    args = parser.parse_args()

    print('Prepare FORMULA pseudo-boxes (COCO2014) in the data format expected from detectron2.')
    ...
    data = []
    cnt = 0
    for image_name in tqdm(FORMULA_pseudo_boxes):
        ...
        data.append({
            ...
            "annotations": prepare_annotation_data(FORMULA_pseudo_boxes[image_name_init]),
        })
        cnt += 1

    print(f'Number images saved {cnt}')
    dataset_name = f"coco20k_train_FORMULA_CAD"
    json_data = {
        "dataset": data,
        "meta_data": {
