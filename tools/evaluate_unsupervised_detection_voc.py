...
if __name__ == '__main__':
    ...
    parser.add_argument('--results', type=str, default='./Pascal_Dino_ResNet50_faster_c4_voc07_based_on_FORMULA_pseudo_boxes_clustered_with_k20/inference/coco_instances_results_voc_2007_test.json')
    args = parser.parse_args()

    meta = MetadataCatalog.get(args.dataset)
    # Too many tiny files, download all to local for speed.
    annotation_dir_local = PathManager.get_local_path(
        os.path.join(meta.dirname, "VOCdevkit", "VOC2007", "Annotations/"))
    args._anno_file_template = os.path.join(annotation_dir_local, "{}.xml")
    args._image_set_path = os.path.join(meta.dirname, "VOCdevkit", "VOC2007" , "ImageSets", "Main", meta.split + ".txt")
