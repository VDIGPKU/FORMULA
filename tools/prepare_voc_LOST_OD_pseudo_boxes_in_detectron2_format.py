...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Prepares the clustered FORMULA pseudo-boxes from the VOC07 "
    ...
    parser.add_argument("--pboxes", type=str, default='',
                        help="Path to where the FORMULA clustered pseudo boxes for the VOC2007 trainval data are.")
    ...
    cluster_ids = [v["pseudo_label"] for v in FORMULA_pseudo_boxes.values() if v != {}]
    num_clusters = max(cluster_ids) + 1
    cluster_to_cls = None

    data = []
    cnt = 0
    for file_name in FORMULA_pseudo_boxes.keys():
        ...
        data.append({
            ...
            "annotations": prepare_annotation_data(FORMULA_pseudo_boxes[file_name], cluster_to_cls),
        })
        cnt += 1
    print(f'Number images saved {cnt}')
    dataset_name = f"voc_2007_trainval_FORMULA_OD_clu{num_clusters}"
    json_data = {
        "dataset": data,
        "meta_data": {
            "dirname": voc_dir,
