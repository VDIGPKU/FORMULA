...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("Unsupervised object discovery with FORMULA.")
    parser.add_argument(
        "--arch",
        default="vit_small",
        type=str,
    ...
    weights = multi_scale_weights[args.dataset]['FORMULA-L']
            ...
            if "vit" in args.arch:
                ...
                if args.dinoseg:
                    ...
                    qkv = (
                        ...
                        # (1 * feat_out["qkv"] + 0 * feat_out["qkv2"] + 0 * feat_out["qkv3"] + 0 * feat_out["qkv4"])
                        .reshape(nb_im, nb_tokens, 3, nh, -1 // nh)
                        .permute(2, 0, 3, 1, 4)
                    )
                    q, k, v = qkv[0], qkv[1], qkv[2]
                ...
                raise ValueError("Model error, only support ViT now.")

        # ------------ Apply FORMULA -------------------------------------------
        if not args.dinoseg:
            pred, A, scores, seed = FORMULA(
                feats,
                feats_init,
                [w_featmap, h_featmap],
                scales,
